...
def simple_fake_response():
    r = fudge.Fake()
    r.has_attr(headers=JSON_HEADERS, text=json.dumps({}))
    fudge.clear_calls()
    return r
...
def testable_dolt():
    ...
    requests = mox.MockObject(original_requests)
    dolt = Dolt(requests=requests)
    return dolt

def dolt_request(dolt, url, method='GET', body=None, headers={}, response_headers=JSON_HEADERS, response_body='{}'):
    response = fudge.Fake()
    response.has_attr(headers=response_headers, text=response_body)
    fudge.clear_calls()
    return (getattr(dolt._requests, method.lower())
            (url, data=body, headers=headers).AndReturn(response))
...
class TestOfTwitterAPI(unittest.TestCase):
    ...
    def test_requests_can_be_passed_in(self):
        requests = "requests-%s" % random.randint(1, 10)
        twitter = Twitter(requests=requests)
        self.assertEqual(twitter._requests, requests)
...
class TestOfCouchDBApi(unittest.TestCase):
    ...
    def test_handles_json_in_get_params(self):
        import requests
        http = mox.MockObject(requests)
        http.get(
            'http://localhost:5984/example/foo?' \
            'startkey=["foo",0]&endkey=["foo",{}]',
            data=None, headers={}
        ).AndReturn(simple_fake_response())
        url = "example"
        couchdb = CouchDB(url, requests=http)
...
class TestOfDolt(unittest.TestCase):
    ...
    def test_get_url_returns_current_attrs(self):
        ...
        replay_all(dolt._requests)

        getattr(getattr(dolt, attr), attr)()
        verify_all(dolt._requests)
    ...
    def test_uses_requests_if_not_provided(self):
        import requests
        dolt = Dolt()
        self.assert_(dolt._requests is requests)
    ...
    def test_takes_an_requests_keyword_arg_and_sets_requests_to_it(self):
        random_requests = "requests-%s" % random.randint(1, 10)
        dolt = Dolt(requests=random_requests)
        self.assertEqual(dolt._requests, random_requests)
